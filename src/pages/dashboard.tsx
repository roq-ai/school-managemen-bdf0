...
import { AcademicYearListPage as ShowCaseList } from 'pages/academic-years';
...
function DashboardPage() {
  ...
  const academicYearStat = useSWR<PaginatedInterface<AcademicYearInterface>>(
    () => `/academic-years`,
    () => getAcademicYears({ limit: 1, offset: 0 }),
  );

  const teacherStat = useSWR<PaginatedInterface<TeacherInterface>>(
    () => `/teachers`,
    () => getTeachers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: academicYearStat?.data?.totalCount,
      isLoading: academicYearStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Teachers',
      result: teacherStat?.data?.totalCount,
      isLoading: teacherStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`academic-years`} passHref legacyBehavior>
                  <Button
