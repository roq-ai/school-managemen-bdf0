...
type ColumnType = ColumnDef<StudentInterface, unknown>;
...
export function StudentListPage(props: StudentListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getStudents({
        relations: ['user', 'academic_year', 'schedule.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['class.contains', 'section.contains', 'transport_details.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StudentInterface>>(
    () => `/students?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StudentInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'transport_details', header: 'Transport Details', accessorKey: 'transport_details' },
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('academic_year', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'academic_year',
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/academic-years/view/${record.academic_year?.id}`}
            >
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/students/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
