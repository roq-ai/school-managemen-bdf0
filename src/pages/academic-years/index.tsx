...
import { getAcademicYears, deleteAcademicYearById } from 'apiSdk/academic-years';
import { AcademicYearInterface } from 'interfaces/academic-year';

type ColumnType = ColumnDef<AcademicYearInterface, unknown>;
...
export function AcademicYearListPage(props: AcademicYearListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getAcademicYears({
        relations: ['organization', 'student.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<AcademicYearInterface>>(
    () => `/academic-years?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: AcademicYearInterface) => {
    if (hasAccess('academic_year', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/academic-years/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('student', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Student',
          accessorKey: 'student',
          cell: ({ row: { original: record } }: any) => record?._count?.student,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/academic-years/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('academic_year', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/academic-years/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('academic_year', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      ...
      >
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="academic_year" />
        </Flex>

        {hasAccess('academic_year', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/academic-years/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
