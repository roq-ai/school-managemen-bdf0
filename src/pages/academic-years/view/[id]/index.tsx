...
import { getAcademicYearById } from 'apiSdk/academic-years';
import { AcademicYearInterface } from 'interfaces/academic-year';
import { StudentListPage } from 'pages/students';

function AcademicYearViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AcademicYearInterface>(
    () => (id ? `/academic-years/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('academic_year', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/academic-years/edit/${id}`} passHref legacyBehavior>
                    <Button
